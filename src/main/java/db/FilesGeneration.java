...
public class FilesGeneration {
	...
	private static final String GAMERS_FILE = "d:/avbc/Documents/TelRun/db/gamer.csv";
	private static final String GAMES_FILE = "d:/avbc/Documents/TelRun/db/games.csv";
	private static final int MAX_GAME_GAMERS = 3;
	private static final String GAMES_GAMERS_FILE = "d:/avbc/Documents/TelRun/db/game-gamer.csv";
	private static final int MAX_MOVES = 10;
	private static final String MOVES_FILE = "d:/avbc/Documents/TelRun/db/moves.csv";
	static Gamer[] gamers;
	...
	private static void createMoves() {
		gameGamerMovesMap = gameGamersMap.keySet()
				.stream()
				.collect(Collectors.toMap(Function.identity(), FilesGeneration::getMoves));
	...
	private static void fillMoves(Move[] moves, String sequenceToBeGuessed, Long gameGamerId, boolean isWinner) {
		...
		IntStream.range(0, lastIndex).forEach(i -> moves[i] = getOneNotFinalMove(sequenceToBeGuessed, gameGamerId));
	...
	private static void createGamesGamers() {
		Map<Long, String[]> gameGamers = getGameGamerNamesMap();
	...
	private static Map<Long, GameGamer> getGameGamersMap(Map<Long, String[]> gameGamers) {
		...
		return gameGamers.entrySet().stream()
				.flatMap(e -> Arrays.stream(e.getValue()).map(gamer -> new GameGamer(gameGamerId++, e.getKey(),
	...
	private static void createGamers() {
		gamers = IntStream.rangeClosed(1, N_GAMERS)
				.mapToObj(FilesGeneration::getRandomGamer)
				.toArray(Gamer[]::new);
